...
class Deprojection(Node):
    ...
    # === Service callbacks (no bbox validation, full image) ===
    ...
    def left_get_object_locations(self, request, response):
        ...
        # Use full image
        color_image_bgr = self.left_color_image.copy()
        ...
        for i in range(len(phrases)):
            ...
            # Center of the box in full image coordinates
            ...
            object_position.x_min = int(xyxy[i][0])
            object_position.y_min = int(xyxy[i][1])
    ...
    def right_get_object_locations(self, request, response):
        ...
        # Use full image
        color_image_bgr = self.right_color_image.copy()
        color_image_rgb = cv2.cvtColor(color_image_bgr, cv2.COLOR_BGR2RGB)
        cv2.imwrite(self.source_image_path, color_image_rgb)
